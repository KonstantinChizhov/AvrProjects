...
bool Crc16::VerifyChecksum(const unsigned char* message, unsigned long length)
{
...
unsigned short Crc16CCITT(const unsigned char *pcBlock, unsigned short len )
{
    unsigned short crc = 0xFFFF;
    unsigned char i;
...
unsigned short table(unsigned char x)
{
    unsigned    value;
...
unsigned short usbCrc16(unsigned char *argPtr, unsigned char argLen)
{
    unsigned crc = 0xffff;

    while(argLen--)
        crc = table((crc&0x00ff) ^ *argPtr++) ^ (crc>>8);
    return ~crc;
}

unsigned short table2(unsigned char x)
{
    unsigned short v = x;
    for (uint8_t i = 8; i--; )
...
#define CRC2(crcval,newchar) crcval = (crcval >> 8) ^ table2((crcval ^ newchar) & 0x00ff)

unsigned short Checksum(const unsigned char* message,
                               unsigned long length,
                               unsigned short crc)
{
    for(unsigned long i = 0; i < length; i++)
...
{
    unsigned char val[2]={128, 0x50};

    std::cout << std::hex << Crc16::Checksum(val, 1) << std::endl;
    std::cout << std::hex << Checksum(val, 1, 0xffff) << std::endl;
    std::cout << std::hex << usbCrc16(val, sizeof(val)) << std::endl;
    std::cout << std::hex << Crc16CCITT(val, sizeof(val)) << std::endl;
